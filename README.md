# 📈 FinAlert - Yapay Zeka Destekli Finans Bildirimi Sistemi
...
**Seçenek B: Railway/Supabase**
- Railway: https://railway.app (500 MB ücretsiz)
- Supabase: https://supabase.com (500 MB ücretsiz)
...
4. **Environment Variables** ekleyin (yukarıdaki .env değişkenleri)
5. **Deploy** butonuna basın!

#### 4. Veritabanı Migration

```bash
# Lokal'de Vercel DATABASE_URL ile
npx prisma migrate deploy
# veya
npx prisma db push
```

Detaylı deployment rehberi: [PRODUCTION-DEPLOYMENT.md](PRODUCTION-DEPLOYMENT.md)
...
# Bülten oluşturma testi
node test-bulletin.js

# AI özet testi
node test-ai-summary.js

# Tam sistem testi
node test-full-bulletin.js
```
...
```bash
# Kayıt olma
curl -X POST http://localhost:3005/api/subscribe \
...
# OTP gönderme
curl -X POST http://localhost:3005/api/preferences/send-otp \
  -H "Content-Type: application/json" \
